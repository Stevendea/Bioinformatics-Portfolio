#!/bin/bash
# Individual RNA-seq processing script version 1.0
# Author: Steven Dea
...
# 1. Fastq file to be analyzed ($1)
# 2. Path to the .gtf file for Hg38 - this file is used for stringtie and featurecounts and should contain gene names ($2)
# 3. Path to the .fa reference genome file for Hg38 - this file is used for HISAT2 alignment ($3)
# 4. # of threads/cores to use for processing ($4)
...
fq=${1?Error: no fastq file specified}

# Grab the base of the file name for output naming
base=`basename $fq .fastq`

# Assigns second positional paramter to the variable gtf
gtf=${2?Error: no .gtf file specified}

# Assigns third positional paramter to the variable ref
ref=${3?Error: no reference genome specified}

# Assigns fourth positional parameter to variable cores, if none is specified default to 1
cores=${4:-1}
...
printf "\nProcessing file: $fq"
printf "\nPerforming fastqc on file: $fq"

# Fastqc on our fastq file
fastqc $fq
...
hisat2 -p $cores --rg-id=$base --rg PL:ILLUMINA -x $ref -U $fq -S $adirbase/$base.sam
printf "\nHISAT2 Alignment of $fq completed\n"
...
# # *******Currently removed as it is not necessary and not working at the moment*******
# # FeatureCounts on the aligned bam file 
# # -T: Specifies the amount of cores to use in this process
# # -a: Specifies the .gtf file
# # -o: Specifies the name of the output file that contains the readcounts. A separate summary statistics file will also be generated
# printf "\nPerforming FeatureCounts on $base.bam"
# featurecounts -T $cores -a $gtf -o $edirbasefc/counts.txt $adirbase/$base.bam
# printf "\nFeatureCounts on $base.bam completed\n"
# # **************************************************************************************
...
# End individual fastq processing script 